...
class RbHotCommentScreenShot(object):
    ...
    def search_note2(self, nickname, title, comment_content):
        '''
                :param keyword: 关键词
                :param nickname: 小红书昵称
                :param title: 笔记标题
        ...
        try:
            ...
            if not self.device(className='android.widget.EditText').exists():
                infor = '输入框获取失败'
                return infor
            self.device(className='android.widget.EditText').send_keys(nickname)
            self.device(text='搜索').click()
            time.sleep(2)
            if not self.device(text='用户').exists():
                infor = '小红书:用户两个字不存在'
                return infor
            self.device(text='用户').click()
            time.sleep(1)
            self.device(resourceId='com.xingin.xhs:id/bb0').click()
            time.sleep(1)
            num = 0
            while True:
                if num > 20:
                    ...
                    return infor
                num += 1
                for nick in self.device(resourceId='com.xingin.xhs:id/cto'):
                    nick.click()
                    # 判断是否是视频
                    if self.device(resourceId='com.xingin.xhs:id/xx').exists():
                        ...
                        for item in range(2):
                            for info in self.device(resourceId='com.xingin.xhs:id/crz'):
                                if comment_content in info.info['text']:
                        ...
                        time.sleep(1)
                        continue
                    if not self.device(text=title).exists():
                        self.device.press('back')
                        time.sleep(1)
                        continue
                    else:
                ...
                if text:
                    ...
                    # 下移
                    self.device.swipe_ext('up', scale=0.4)
                    time.sleep(1)
            if not text:
                infor = '搜索达到上限'
                return infor
        ...
        while True:
            try:
                ...
                if text['Id'] == 0:
                    ...
                    continue
                if text['Title']:
                    str2 = RbHotCommentScreenShot.search_note2(self, text['NickName'], text['Title'],
                                                               text['CommentContent'])
                else:
